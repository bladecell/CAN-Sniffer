<script lang="ts">
  ...
  function handleToggle(event: Event) {
    ...
    // Trigger the custom action if one was passed in
    if (onchange) {
      onchange(checked);
    }
  }
...
<div class="toggle-container" class:has-label={!!label}>
  {#if label}
    <div class="toggle-text-group">
      <div class="toggle-label">{label}</div>
      {#if subLabel}
        <div class="toggle-sub-label">{subLabel}</div>
      {/if}
    </div>
  {/if}
...
<style>
  ...
  /* Expands to fill width and separate text/switch if a label exists */
