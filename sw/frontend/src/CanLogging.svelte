<script>
  ...
  const MIN_SIZES = { card: { w: 12, h: 7 }, plot: { w: 24, h: 12 } };
  const MAX_SIZES = {
    card: { w: MIN_SIZES.card.w * 1.5, h: MIN_SIZES.card.h * 1.5 },
    plot: { w: MIN_SIZES.plot.w * 1.5, h: MIN_SIZES.plot.h * 1.5 },
  };
        ...
        {#each items as item (item.id)}
          ...
          >
            {#snippet children()}
              ...
              >
                {#if item.displayMode === "card"}
                  <PIDCard {...item.cardData} />
                {:else}
                  <!-- <PIDPlot {...item.cardData} /> -->
                {/if}
              </div>
            {/snippet}
          </GridItem>
...
{#if contextMenu.show}
  ...
  >
    ...
    <button
      onclick={() => {
        console.log("Edit properties for:", contextMenu.targetId);
        closeMenu();
      }}
    >
      Edit Properties
