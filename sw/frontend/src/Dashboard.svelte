<script>
  ...
  const MAX_SIZES = {
    ...
    bar: { w: MIN_SIZES.gauge.w * 1.5, h: MIN_SIZES.gauge.h * 1.5 },
  };
  ...
  function toggleMode(id) {
    ...
    if (item) {
      const modes = ["card", "chart", "gauge", "bar"];
      const current = modes.indexOf(item.displayMode);
      item.displayMode = modes[(current + 1) % modes.length];
      items = [...items];
    }
    closeMenu();
  }
